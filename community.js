(function () {
  ...
  function generateChatActivity() {
    if (!chatArea || chatArea.offsetParent === null || !window._chatActive) return;
  ...
  window.initCommunityChat = function() {
    if (!chatArea) return;
    const existingHeading = chatArea.querySelector('.tab-heading');
    if (!existingHeading) {
      chatArea.innerHTML = '';
      populateInitialChat();
    } else {
      chatArea.innerHTML = '';
      chatArea.appendChild(existingHeading);
    }
    if (!window._chatLoopStarted) {
      window._chatLoopStarted = true;
      window._chatActive = true;
      window._chatLoopId = setInterval(() => {
        if (chatArea && chatArea.offsetParent !== null && window._chatActive) {
  ...
  console.log('RemitFlow AI community loaded – 351 personas, slang, typos, replies, join messages');
})();
