(function () {
  ...
  // ---------- EXPOSE GLOBALLY FOR MEMBERS LIST ----------
  window._communityPersonas = personas;
  ...
  function applySlang(text, lang) {
    if (lang === 'bengali') return text.replace(/I /g, 'Ami ').replace(/my /g, 'amar ').replace(/ brother/, ' bhai') + ' 🇧🇩';
    if (lang === 'arabic') return text.replace(/I /g, 'Ana ').replace(/money/, 'floos') + ' الحمد لله';
    if (lang === 'mixed') return text.replace(/I /g, 'Me ').replace(/just /, 'just dey ').replace(/saved/, 'save') + ' oo';
    return text;
  }
  ...
  const recentMessages = [];                // last 20 plain texts (for reply context)
  const chatMessageHTMLBuffer = [];         // full HTML buffer (capped at 500)
  const MAX_CHAT_BUFFER = 500;
  const typingTimeouts = [];

  // ==================== BUFFERED MESSAGE POST ====================
  function postMessage(persona, isReply = false, originalMsg = null, timestampOffset = 0) {
    ...
    // Buffer (capped)
    chatMessageHTMLBuffer.push(msgHTML);
    while (chatMessageHTMLBuffer.length > MAX_CHAT_BUFFER) chatMessageHTMLBuffer.shift();

    // Append to DOM ONLY when General Chat tab is active
    if (window._chatActive && chatArea) {
      chatArea.insertAdjacentHTML('beforeend', msgHTML);
      chatArea.scrollTop = chatArea.scrollHeight;
    }

    // Keep recent messages for reply context
  ...
  // Typing indicator – only shown when General Chat is active
  let typingCount = 0;
  const MAX_TYPERS = 2;
  function showTypingIndicator(persona, isReply = false, originalMsg = null) {
    if (!chatArea || !window._chatActive) return;
  ...
  // Continuous activity – always runs, messages go to buffer
  function generateChatActivity() {
    ...
    if (window._chatActive && typingCount < MAX_TYPERS) {
      showTypingIndicator(persona, shouldReply, originalMsg);
    } else {
      postMessage(persona, shouldReply, originalMsg, 0);
    }

    if (window._chatActive && Math.random() < 0.3 && typingCount < MAX_TYPERS) {
    ...
    } else if (!window._chatActive && Math.random() < 0.3) {
      let persona2 = personas[randomInt(0, personas.length - 1)];
      postMessage(persona2, false, null, 0);
    }
  }
  ...
  window._chatLoopStarted = false;

  window.initCommunityChat = function() {
    if (!chatArea) return;
    if (!window._chatLoopStarted) {
      window._chatLoopStarted = true;
      // Seed the buffer if empty
      if (chatMessageHTMLBuffer.length === 0) {
        for (let i = 19; i >= 0; i--) {
          const p = personas[randomInt(0, personas.length - 1)];
          const isReply = recentMessages.length > 0 && Math.random() < 0.25;
          let orig = null;
          if (isReply) {
            orig = recentMessages[randomInt(0, recentMessages.length - 1)];
            while (orig && orig.name === p.name) orig = recentMessages[randomInt(0, recentMessages.length - 1)];
          }
          postMessage(p, isReply, orig, 10 + i * 2);
        }
      }
      window._chatLoopId = setInterval(() => {
        if (chatArea) generateChatActivity();
      }, 3500);
    }

    // Switch to active and render buffer
    window._chatActive = true;
    if (chatArea) {
      const heading = chatArea.querySelector('.tab-heading');
      chatArea.innerHTML = '';
      if (heading) chatArea.appendChild(heading);
      chatArea.insertAdjacentHTML('beforeend', chatMessageHTMLBuffer.join(''));
      chatArea.scrollTop = chatArea.scrollHeight;
    }
  };
  ...
  window.stopCommunityChat = function() {
    ...
    typingCount = 0;
    // Buffer is kept – history preserved
  };

  // ==================== LIVE STATS ====================
  if (onlineCountEl) {
    setInterval(() => { onlineCountEl.textContent = randomInt(400, 800).toString(); }, 30000);
  }
  if (liveFeeValEl) {
    setInterval(() => { liveFeeValEl.textContent = `$${(Math.random() * 10 + 1).toFixed(2)}`; }, 5000);
  }

  console.log('RemitFlow AI community loaded – 351 personas, persistent chat, buffered messages, no bleeding');
})();
