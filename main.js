(function () {
  ...
  // ---------- MEMBERS (12,450 unique names, pre‑cached, no duplicates, no numbers) ----------
  const TOTAL_MEMBERS = 12450;

  // 200 first names × 200 last names = 40,000 unique combos → no duplicates possible
  const FIRST_NAMES = [
    'Abigail','Carlos','Deepak','Elena','Fatou','Hassan','Ingrid','Jin','Kwesi','Laila',
    'Mikhail','Nia','Omar','Priyanka','Ravi','Sofia','Tariq','Uche','Vera','Wei',
    'Amara','Babatunde','Camila','Dmitri','Esperanza','Farid','Gita','Hiroshi','Imani','Jamal',
    'Kiran','Luis','Mei','Nikolai','Olga','Pablo','Qiana','Raj','Sakura','Thabo',
    'Umar','Valentina','Wanjiru','Xavier','Yuki','Zahara','Akira','Bianca','Cheng','Dalia',
    'Aisha','Bamidele','Chioma','Dae-Hyun','Emeka','Fadila','Gustavo','Hannah','Idris','Jia',
    'Kofi','Leila','Mohamed','Nalini','Obi','Purnima','Qadir','Rosa','Samir','Thandiwe',
    'Ugo','Viktor','Wen','Ximena','Yara','Zoltan','Adewale','Bahati','Cai','Dakarai',
    'Efe','Fariha','Gafar','Hadiya','Ifeanyi','Jabari','Kadija','Laszlo','Makena','Nasir',
    'Ogechi','Palesa','Qismat','Rudo','Sekou','Tinashe','Uzoma','Vusi','Wambui','Xolani',
    'Yejide','Zanele','Akin','Binta','Chidi','Dalia','Eshe','Fola','Ghalib','Husna',
    'Ikenna','Jelani','Kamau','Lulu','Mosi','Nuru','Omarion','Penda','Qabil','Rami',
    'Sade','Tendai','Udo','Vivian','Wekesa','Xola','Yaa','Zuri','Amir','Bola',
    'Chima','Dumi','Enitan','Faraji','Gugu','Hadi','Ife','Jengo','Kesi','Lumumba',
    'Makenna','Nkosana','Olu','Paki','Qamar','Rafiki','Sefu','Tau','Uchechi','Vuyo',
    'Wanjala','Xola','Yonas','Zola','Ayo','Bhekisisa','Chinua','Dikeledi','Ekene','Fumnanya'
  ];

  const LAST_NAMES = [
    'Santos','Mendoza','Singh','Petrova','Diop','Ali','Svensson','Park','Asare','Nur',
    'Volkov','Johnson','Farouk','Das','Kumar','Andersson','Mahmoud','Okonkwo','Ivanova','Chen',
    'Williams','Brown','Garcia','Martinez','Robinson','Clark','Rodriguez','Lewis','Lee','Walker',
    'Hall','Allen','Young','King','Wright','Scott','Green','Adams','Baker','Nelson',
    'Hill','Rivera','Campbell','Mitchell','Carter','Roberts','Phillips','Evans','Turner','Collins',
    ...
    'Henry','Boyd','Mason','Morales','Kennedy','Warren','Dixon','Reyes','Burns','Gordon',
  ...
  function buildFullMemberList() {
    ...
    // 1. insert real community personas (unique names)
    ...
    // 2. fill remaining with random unique first+last
    const needed = TOTAL_MEMBERS - members.length;
    let attempts = 0;
    const maxAttempts = needed * 20;  // safety margin
    while (members.length < TOTAL_MEMBERS && attempts < maxAttempts) {
  ...
  // Pre‑cache asynchronously
  function generateAllMembersAsync(callback) {
    if (window._allMembersCache) {
      callback(window._allMembersCache);
      return;
    }
    // Use a timeout to avoid freezing the UI during generation
    setTimeout(() => {
      const list = buildFullMemberList();
      window._allMembersCache = list;
      callback(list);
    }, 50);
  }
  ...
  function renderMembersTab(loadAll = false) {
    ...
    // Kill any chat loop that might be running
    if (window._chatLoopId) {
      clearInterval(window._chatLoopId);
      window._chatLoopId = null;
    }
    ...
    if (!loadAll) {
      ...
      chatArea.innerHTML = `
        ...
        <div style="text-align:center;padding:30px 10px;">
          ...
          <div style="font-weight:600;font-size:1rem;margin-bottom:4px;">${TOTAL_MEMBERS} Members</div>
    ...
    // Load full list
    chatArea.innerHTML = `<div style="padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">👥 All ${TOTAL_MEMBERS} Community Members</div>
    <div style="text-align:center;padding:20px;color:var(--text3);">Loading members...</div>`;

    generateAllMembersAsync(function(allMembers) {
      chatArea.innerHTML = `<div style="padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">👥 All ${TOTAL_MEMBERS} Community Members</div>`;
      const chunkSize = 50;
      let i = 0;
  ...
  window.switchCommunityTab = function(tab) {
    ...
    // ALWAYS kill chat loop before switching
    if (window._chatLoopId) {
      clearInterval(window._chatLoopId);
      window._chatLoopId = null;
    }
  ...
  function initApp() {
    ...
    renderTransactions();
    bindEvents();
    // Pre‑cache the full member list in background
    generateAllMembersAsync(function(){});
    if (typeof lucide !== 'undefined') lucide.createIcons();
