(function () {
  ...
  function generateAllMembers() {
    ...
    while (allMembers.length < TOTAL_MEMBERS) {
  ...
  function getFullMemberList() {
    if (!window._allMembersCache) {
      window._allMembersCache = generateAllMembers();
    }
    return window._allMembersCache;
  }
  ...
  // ==================== SUCCESS STORIES (green border style) ====================
  ...
  function initApp() {
    ...
    renderTransactions();
    bindEvents();
    // Pre‑warm the full member list in the background
    setTimeout(() => getFullMemberList(), 100);
    if (typeof lucide !== 'undefined') lucide.createIcons();
