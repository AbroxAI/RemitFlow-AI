(function () {
  ...
  function generateAllMembersAsync(callback) {
    ...
    const baseNames = [
      'Abigail Santos','Carlos Mendoza','Deepak Singh','Elena Petrova','Fatou Diop',
      'Hassan Ali','Ingrid Svensson','Jin Soo Park','Kwesi Asare','Laila Nur',
      'Mikhail Volkov','Nia Johnson','Omar Farouk','Priyanka Das','Ravi Kumar',
      'Sofia Andersson','Tariq Mahmoud','Uche Okonkwo','Vera Ivanova','Wei Chen',
      'Amara Williams','Babatunde Brown','Camila Garcia','Dmitri Martinez','Esperanza Robinson',
      'Farid Clark','Gita Rodriguez','Hiroshi Lewis','Imani Lee','Jamal Walker',
      'Kiran Hall','Luis Allen','Mei Young','Nikolai King','Olga Wright',
      'Pablo Scott','Qiana Green','Raj Adams','Sakura Baker','Thabo Nelson',
      'Umar Hill','Valentina Rivera','Wanjiru Campbell','Xavier Mitchell','Yuki Carter',
      'Zahara Roberts','Akira Phillips','Bianca Evans','Cheng Turner','Dalia Collins'
    ];
    ...
    function processChunk() {
      const chunkSize = 100;
      const end = Math.min(counter + chunkSize, totalNeeded);
      for (; counter < end; counter++) {
        const baseName = baseNames[counter % baseNames.length];
        const uniqueName = `${baseName} ${counter + 1}`;
        const initials = baseName.split(' ').map(n => n[0]).join('').toUpperCase();
        allMembers.push({
          name: uniqueName,
          initials,
          online: Math.random() < 0.3,
  ...
  // Members tab – only a “View All” button initially
  function renderMembersTab(loadAll = false) {
    ...
    // Load full list (show a loading message while waiting)
    ...
    getFullMemberList(function(allMembers) {
      ...
      const chunkSize = 50;   // 50 members at a time
      let i = 0;
  ...
  // (No more 80‑member simulation, so no setInterval needed)

  // ==================== SUCCESS STORIES (green border cards) ====================
  ...
  window.switchCommunityTab = function(tab) {
    ...
    switch (tab) {
      ...
      case 'help':
        ...
        break;
      case 'members':
        renderMembersTab(false);   // show button, not the full list
        break;
      default:
        chatArea.innerHTML = `<div style="text-align:center;padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">${tabHeadings.general}</div>`;
        if (typeof window.initCommunityChat === 'function') window.initCommunityChat();
  ...
  function initApp() {
    ...
    renderTransactions();
    bindEvents();
    // Pre‑generate the full member list in the background (non‑blocking)
