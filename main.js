(function () {
  ...
  // ==================== MEMBERS LIST – 12,450 UNIQUE NAMES ====================
  ...
  function generateAllMembersAsync(callback) {
    ...
    const totalNeeded = TOTAL_MEMBERS - allMembers.length;
    let generated = 0;
    const MAX_ATTEMPTS = totalNeeded * 10;

    function processChunk() {
      const chunkSize = 100;
      let remaining = chunkSize;
      let attempts = 0;
      while (generated < totalNeeded && remaining > 0 && attempts < MAX_ATTEMPTS) {
      ...
      if (generated < totalNeeded && attempts < MAX_ATTEMPTS) {
        setTimeout(processChunk, 0);
      } else {
        allMembers.sort((a,b) => (a.online && !b.online ? -1 : (!a.online && b.online ? 1 : a.name.localeCompare(b.name))));
        window._allMembersCache = allMembers;
        callback(allMembers);
      }
    }
    processChunk();
  ...
  function getFullMemberList(callback) {
    ...
    generateAllMembersAsync(callback);
  }
