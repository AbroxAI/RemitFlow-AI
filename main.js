(function () {
  ...
  // ==================== MEMBERS – 12,450 UNIQUE NAMES, NO NUMBERS ====================
  ...
  function buildFullMemberList() {
    ...
    // Insert real community personas first
    ...
    // Fill remaining from shuffled combos
    for (const combo of allCombos) {
      if (members.length >= TOTAL_MEMBERS) break;
  ...
  function renderMembersTab(loadAll = false) {
    ...
    // Kill chat loop completely
    if (typeof window.stopCommunityChat === 'function') window.stopCommunityChat();
  ...
  window.switchCommunityTab = function(tab) {
    ...
    // Always stop chat before switching
    if (typeof window.stopCommunityChat === 'function') window.stopCommunityChat();
    chatArea.innerHTML = '';
