(function () {
  ...
  function renderSuccessTab() {
    ...
    if (!chatArea) return;
    const stories = [
      { name: 'Chidi Eze', location: 'Dubai, UAE', text: 'I used to lose $15 every month on fees sending money to Lagos. Now I earn from those same transactions. My family finally receives the full amount.', time: '2 hours ago' },
      { name: 'Amara Obi', location: 'London, UK', text: 'Started with 5 USDT just to test. Two months later, my fee share covers my parents’ monthly bills in Abuja.', time: '5 hours ago' },
      { name: 'Emeka Nnamdi', location: 'New York, USA', text: 'I send money to my relatives in Lagos every month. RemitFlow has saved me over $200 in fees this year alone.', time: '8 hours ago' },
      { name: 'Fatima Al-Hashimi', location: 'Abu Dhabi, UAE', text: 'The platform is a blessing. My sister in London uses it to send money home without any charges.', time: '1 day ago' },
  ...
  function renderAnnouncementsTab() {
    ...
    const announcements = [
      ...
      { text: '💸 New remittance corridor: Canada → Nigeria now active.', time: '3 days ago' },
      ...
      { text: '🌐 USDT/NGN live spread now displayed on dashboard.', time: '1 week ago' },
  ...
  // ==================== FEE CAPTURES (Nigerian + global pairs) ====================
  function renderFeeCapturesTab() {
    const chatArea = document.getElementById('chatArea');
    if (!chatArea) return;
    const pairs = ['USD/NGN','AED/NGN','GBP/NGN','EUR/NGN','CAD/NGN','USD/PHP','AED/PHP','GBP/PHP','MYR/PHP','SGD/PHP'];
