(function () {
  ...
  function buildMemberList() {
    ...
    const total = 80;
    const members = [];
    for (let i = 0; i < Math.min(all.length, total); i++) {
      const p = all[i];
      members.push({
        name: p.name,
        initials: p.initials,
  ...
  function renderMembersTab() {
    ...
    if (!chatArea) return;
    if (memberList.length === 0) memberList.push(...buildMemberList());
    let html = `<div style="padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">👥 Community Members (${memberList.length})</div>`;
      ...
      html += `
        <div style="background:var(--surface2);border-radius:10px;padding:10px;margin-bottom:8px;border-left:3px solid var(--green);">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
            <div style="font-weight:700;color:var(--gold);">${t.name}</div>
            <div style="font-size:0.65rem;color:var(--text3);">${t.location}</div>
          </div>
          <div style="font-size:0.75rem;color:var(--text2);">“${t.text}”</div>
        </div>`;
    });
    chatArea.innerHTML = html;
  }
  ...
  function renderMarketTab() {
    const chatArea = document.getElementById('chatArea');
    if (!chatArea) return;
    let html = `<div style="padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">📢 Announcements & Updates</div>`;

    // Extended announcements
    const announcements = [
      '🚀 RemitFlow now supports BEP20 USDT for fee captures!',
      '📊 July fee shares have increased by 8% across all tiers.',
      '🌍 Diaspora Tier now available for liquidity above $500.',
      '🔒 Enhanced security: 2FA is now live in wallet settings.',
      '💸 New remittance corridor: Canada → Bangladesh now active.',
      '🏦 Partnered with 3 new local banks for faster settlements.',
      '🎉 Referral bonus doubled – invite friends and earn 10% extra!',
      '📱 Mobile app coming soon – early access for Premium members.',
      '🌐 USDT/BDT live spread now displayed on dashboard.',
      '✅ Withdrawals processed in under 10 minutes – 99.9% uptime.',
      '📈 Total fee shares distributed this week: $1.2M+'
    ];
    announcements.forEach(a => {
      html += `<div style="font-size:0.8rem;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05);">${a}</div>`;
    });

    // Fee captures (same as home page)
    html += `<div style="margin-top:12px;font-weight:600;font-size:0.8rem;color:var(--gold);">Recent Fee Captures</div>`;
    ...
    for (let i=0; i<4; i++) {
      ...
      html += `
        <div style="display:flex;justify-content:space-between;align-items:center;background:var(--surface2);padding:8px;border-radius:8px;margin-top:6px;">
          <div>
            <div style="font-weight:600;font-size:0.75rem;">${pair} spread – captured $${cap}</div>
  ...
  function renderHelpTab() {
    ...
    let html = `
      ...
      <div style="text-align:center;padding:20px;">
        <i data-lucide="headphones" style="width:32px;height:32px;color:var(--gold);margin-bottom:12px;"></i>
        <div style="font-weight:600;margin-bottom:4px;">Support Team Online</div>
        <div style="font-size:0.75rem;color:var(--text3);margin-bottom:12px;">Describe your issue and we'll respond within 5 minutes.</div>
        <input type="text" placeholder="Type your message..." disabled style="width:100%;padding:10px;background:var(--surface2);border:1px solid var(--surface3);border-radius:20px;color:var(--text);font-family:Inter;">
        <div style="margin-top:8px;font-size:0.65rem;color:var(--text3);">Login to enable live chat</div>
      </div>`;
    chatArea.innerHTML = html;
    if (typeof lucide !== 'undefined') lucide.createIcons();
  }
  ...
  window.switchCommunityTab = function(tab) {
    ...
    // Stop any existing chat loop (community.js may set a global interval)
  ...
  // Simulate new joiners in members tab
  setInterval(() => {
