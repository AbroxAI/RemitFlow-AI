(function () {
  ...
  // ==================== MEMBERS – 12,450 UNIQUE NAMES, NON‑BLOCKING ====================
  ...
  function buildFullMemberListAsync(callback) {
    ...
    function processChunk(deadline) {
      while (members.length < TOTAL_MEMBERS && comboIndex < allCombos.length) {
        const combo = allCombos[comboIndex++];
        if (!usedNames.has(combo)) {
          const [first, last] = combo.split(' ');
          members.push({
            name: combo,
        ...
        // Yield every 50 combos
        if (comboIndex % 50 === 0 && deadline && deadline.timeRemaining() < 1) {
  ...
  function renderMembersTab(loadAll = false) {
    ...
    getFullMemberList(function(allMembers) {
      // Abort if version changed or tab no longer members
  ...
  // ==================== TAB SWITCHING (active flag fixes bleeding) ====================
  const tabHeadings = {
  ...
  window.switchCommunityTab = function(tab) {
    ...
    // Abort any pending members render
    membersRenderVersion++;

    // *** THIS IS THE FIX: set chat active flag based on tab ***
  ...
  function initApp() {
    ...
    renderTransactions();
    bindEvents();
    // Pre‑warm the member list in the background
