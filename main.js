(function () {
  ...
  function renderMembersTab(showAll = false) {
    ...
    if (showAll) {
      const allMembers = generateAllMembers();
      let html = `<div style="padding:10px;font-weight:700;color:var(--gold);border-bottom:1px solid var(--gold);margin-bottom:8px;">👥 All ${TOTAL_MEMBERS} Community Members</div>`;
    ...
    stories.forEach(s => {
      html += `
        <div style="margin-bottom:12px;border-bottom:1px solid rgba(255,255,255,.05);padding-bottom:8px;">
          ...
          <div style="font-size:0.8rem;color:var(--text2);margin-top:2px;">“${s.text}”</div>
        </div>`;
    });
    chatArea.innerHTML = html;
  }
